...
let
  cfg = config.custom.hardware.openrazer;
...
in
{
  options = {
    custom.hardware.openrazer = {
      ...
      package = lib.mkOption {
        default = kernelPackages.openrazer;
        defaultText = lib.literalExpression ''
          config.boot.kernelPackages.openrazer
        '';
        example = "config.boot.kernelPackages.openrazer.overrideAttrs (oldAttrs: { ... })";
        description = ''
          The openrazer driver package to use.
        '';
      };
    };
  };
  ...
  imports = [
    (lib.mkRenamedOptionModule
      [ "custom" "hardware" "openrazer" "mouseBatteryNotifier" ]
      [ "custom" "hardware" "openrazer" "batteryNotifier" "enable" ]
    )
  ];
